...
import {
  ...
  isSyntheticToolRepairMessage, shouldAutoRecoverMissingToolResults,
  buildSyntheticMissingToolResult, shouldRestartSessionAfterSyntheticToolRepair,
...
import { sendAndWaitNoTimeout, readShutdownMetrics } from './timeout.mjs';
...
async function main() {
  ...
  const getOrCreateSessionEntry = async (
    ...
    const entry = {
      ...
      usageCursor: { inputTokens: 0, outputTokens: 0 },
      clientTools,
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/v1/chat/completions') {
        ...
        if (entry.pendingToolCalls?.size) {
          ...
          if (missingToolCalls.length) {
            if (shouldRestartSessionAfterSyntheticToolRepair(toolMessages, missingToolCalls)) {
              ...
              return sendJson(res, 400, {
                error: { message: `Missing tool results for pending tool call ids: ${missingToolCalls.join(', ')}`, type: 'invalid_request_error' },
              });
            } else {
        ...
        recordCompleted(model, latencyMs, response.usage?.prompt_tokens ?? 0, response.usage?.completion_tokens ?? 0);
        ...
        return sendJson(res, 200, response, responseHeaders);
          ...
          if (_isTimeout && !res.headersSent && retryCount < 1) {
            ...
            if (_staleEntry && !_staleEntry.closing) {
              await closeSessionEntry(_staleEntry, 'silent-retry');
            }
            logProxyEvent('silent_retry', {
              sessionKey: _staleKey, attempt: retryCount + 1,
              code: _re.code, phase: _re.phase ?? null, model: reqModel ?? null,
            });
            reqModel = null;
            reqStartedAt = 0;
